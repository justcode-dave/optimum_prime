"""
This module defines the base classes and configurations used for exporting Hugging Face models to the ONNX format.

The ONNX configuration classes provide metadata and functionality to guide the process of converting models from 
different frameworks (such as PyTorch and TensorFlow) into the ONNX format. The base configurations offer a standardized 
way to define the inputs, outputs, and behaviors of models during export.

Key Features:
- **OnnxConfig**: The core class that defines the structure for exporting models, including input/output definitions, 
  dynamic axes handling, and dummy input generation for export validation.
- **OnnxConfigWithPast**: Extends `OnnxConfig` to handle models with past key-value caching, such as autoregressive 
  text generation models.
- **OnnxSeq2SeqConfigWithPast**: Further extends `OnnxConfigWithPast` for sequence-to-sequence models like 
  transformers with separate encoder and decoder components.
- **OnnxConfigWithLoss**: A configuration wrapper that includes loss computation and label generation for models, 
  especially for training scenarios.
- **ConfigBehavior**: Enumerates the behavior of ONNX configurations, specifying whether they operate on the entire 
  model, the encoder, or the decoder separately.

This module is designed to be highly extensible, allowing customization of dummy input generation, tensor shape 
adjustments, and input/output renaming. The goal is to provide a flexible foundation for exporting various model 
architectures to ONNX, including support for advanced features such as past key-value caching and multi-file model exports.

Usage:
- Subclass the base configurations to create ONNX export configurations tailored to specific models and tasks.
- Use the methods provided to customize input and output handling, including support for dynamic axes.
- The configurations support post-processing steps, such as deduplicating tied weights in the exported models, to 
  improve performance and compatibility with ONNX runtime environments.
"""
