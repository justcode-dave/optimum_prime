"""
This script defines the entry point for the Optimum Command-Line Interface (CLI) tool.

The Optimum CLI provides various commands to interact with the Optimum library, including exporting models and inspecting the environment. It supports dynamic registration of subcommands from subpackages or external files.

The CLI framework uses a root command with subcommands that are defined in various parts of the Optimum package. This script includes functionality to:
- Register top-level subcommands such as "export" and "env".
- Dynamically load subcommands from other modules or packages.
- Provide a structured and extendable CLI interface for Optimum users.

The `main` function is the entry point for the CLI.
"""
